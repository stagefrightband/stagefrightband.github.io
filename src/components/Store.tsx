...
import '../styles.css';
function getCookie(name:string):string|null{const value=`; ${document.cookie}`;const parts=value.split(`; ${name}=`);if(parts.length===2){const part=parts.pop();if(part)return decodeURIComponent(part.split(";").shift()||"");}return null}function setCookie(name:string,value:string):void{const expires="Fri, 31 Dec 9999 23:59:59 GMT";document.cookie=`${name}=${encodeURIComponent(value)}; expires=${expires}; path=/`}interface ToggleClassBasedOnCookieParams{cookieName:string;className:string}function toggleClassBasedOnCookie({cookieName,className}:ToggleClassBasedOnCookieParams):void{const cookieValue=getCookie(cookieName)==="true";if(cookieValue){document.documentElement.classList.add(className)}else{document.documentElement.classList.remove(className)}}const Store:React.FC=()=>{const[isOverlayVisible,setIsOverlayVisible]=useState(false);const[isTicketOverlayVisible,setIsTicketOverlayVisible]=useState(false);const[isAlbumOverlayVisible,setIsAlbumOverlayVisible]=useState(false);const[isCDOverlayVisible, setIsCDOverlayVisible] = useState(false);
const [isDigitalOverlayVisible, setIsDigitalOverlayVisible] = useState(false); // New state for Digital S-Gate
const[quantity,setQuantity]=useState(1);const[selectedVenue,setSelectedVenue]=useState<string>("House of Blues, Houston");const[size,setSize]=useState<string>("M");const[cdQuantity, setCDQuantity] = useState(1);
useEffect(()=>{toggleClassBasedOnCookie({cookieName:"highcontrast",className:"high-contrast"});toggleClassBasedOnCookie({cookieName:"opendyslexic",className:"open-dyslexic"});const handleKeyDown=(e:KeyboardEvent)=>{if(e.key==='Escape'){setIsOverlayVisible(false);setIsTicketOverlayVisible(false);setIsAlbumOverlayVisible(false);setIsCDOverlayVisible(false);setIsDigitalOverlayVisible(false);}};document.addEventListener('keydown',handleKeyDown);return()=>{document.removeEventListener('keydown',handleKeyDown)}},[]);const handleMerchButtonClick=()=>{setIsOverlayVisible(true)};const handleTicketButtonClick=()=>{setIsTicketOverlayVisible(true)};const handleAlbumButtonClick=()=>{setIsAlbumOverlayVisible(true)};const handleCDButtonClick = () => {
  setIsCDOverlayVisible(true);
};
const handleDigitalButtonClick = () => {
  setIsDigitalOverlayVisible(true);
}; // New handler for Digital S-Gate button
const handleQuantityChange=(e:React.ChangeEvent<HTMLInputElement>)=>{const value=parseInt(e.target.value,10);setQuantity(value>=1?value:1)};const handleVenueChange=(e:React.ChangeEvent<HTMLSelectElement>)=>{setSelectedVenue(e.target.value)};interface CartItem{name:string;quantity:number;venue? :string;size? :string}const updateCartItems=(itemName:string,quantityToAdd:number,venue? :string,size? :string)=>{const currentCart=getCookie('cartitems');const itemsMap:Record<string, CartItem>={};if(currentCart){currentCart.split(',').forEach(item=>{const match=item.match(/^([A-Za-z' _-]+)(\d+)(?:_([A-Za-z' _-]+))?$/);if(match){const name=match[1];const quantity=parseInt(match[2],10);const venueName=match[3]?match[3].replace(/_/g,' '):undefined;const key=venueName?`${name}_${venueName}`:name;if(itemsMap[key]){itemsMap[key].quantity+=quantity}else{itemsMap[key]={name,quantity,venue:venueName}}}})}let key=itemName;if(itemName==='Tickets'&&venue){const formattedVenue=venue.replace(/ /g,'_').replace(/,/g,'');key+=`_${formattedVenue}`}if(itemName==='Merch'&&size){key+=`_${size}`}if (itemName === 'Digital S-Gate' || itemName === 'CD: S-Gate') {
    key += ''; // No additional attributes
  }if(itemsMap[key]){itemsMap[key].quantity+=quantityToAdd}else{itemsMap[key]={name:itemName,quantity:quantityToAdd,venue,size}}const updatedCart=Object.values(itemsMap).map(cartItem=>{let itemString=`${cartItem.name}${cartItem.quantity}`;if(cartItem.venue){const formattedVenue=cartItem.venue.replace(/ /g,'_').replace(/,/g,'');itemString+=`_${formattedVenue}`}if(cartItem.size){itemString+=`_${cartItem.size}`}return itemString}).join(',');setCookie('cartitems',updatedCart)};const handleSubmitMerch=()=>{updateCartItems('Merch',quantity,undefined,size);console.log(`Added Merch${quantity}_${size} to cart.`);setIsOverlayVisible(false)};const handleSubmitTickets=()=>{updateCartItems('Tickets',quantity,selectedVenue);console.log(`Added Tickets${quantity}_${selectedVenue.replace(/ /g,'_')} to cart.`);setIsTicketOverlayVisible(false)};const handleSubmitAlbum=()=>{updateCartItems('S-Gate',quantity);console.log(`Added S-Gate${quantity} to cart.`);setIsAlbumOverlayVisible(false)};
const handleSubmitCD = () => {
  updateCartItems('CD: S-Gate', cdQuantity);
  console.log(`Added CD: S-Gate${cdQuantity} to cart.`);
  setIsCDOverlayVisible(false);
};
const handleSubmitDigital = () => {
  updateCartItems('Digital S-Gate', quantity);
  console.log(`Added Digital S-Gate${quantity} to cart.`);
  setIsDigitalOverlayVisible(false);
}; // New submit handler for Digital S-Gate
  return (
    <div>
        ...
        <div className="product-tile">
          <div className="product-image">
            <button
              ...
              aria-label="Digital Version: S-Gate"
            ></button>
          </div>
          <div className="product-info">
            <p>Digital Version: S-Gate</p> 
          </div>
        </div>
        <div className="product-tile">
          <div className="product-image">
      ...
      <div className={`overlay ${isAlbumOverlayVisible ? 'active' : ''}`}>
        ...
        <div className="overlay-left">
          <img src="/Images/albumcover.webp" alt="Digital Version: S-Gate" />
        </div>
        <div className="overlay-right">
          <div className="overlay-content">
            <p>Digital Version: S-Gate</p>
          ...
          <div className="overlay-content">
            ...
            <div className="quantity-container">
              ...
              <input
                ...
                min="1"
                value={cdQuantity}
                onChange={(e) => setCDQuantity(parseInt(e.target.value, 10) || 1)}
              />
            </div>
            <button className="submit-button" onClick={handleSubmitCD}>Add to Cart</button>
          </div>
        </div>
      </div>

      <div className={`overlay ${isDigitalOverlayVisible ? 'active' : ''}`}>
        <button className="close-button" onClick={() => setIsDigitalOverlayVisible(false)}>X</button>
        <div className="overlay-left">
        ...
        <div className="overlay-right">
          <div className="overlay-content">
            ...
            <button className="submit-button" onClick={handleSubmitDigital}>Add to Cart</button>
          </div>
        </div>
      </div>
    </div>
